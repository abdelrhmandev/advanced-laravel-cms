@if (!$field->isTranslatable())
    <flux:input wire:model="formValues.{{ $pivotId }}.{{ $row }}.{{ $field->id }}"
        id="icon-picker-{{ $pivotId }}_{{ $row }}_{{ $field->id }}" data-icon-picker
        ...
        @foreach (\Mcamara\LaravelLocalization\Facades\LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            <div class="space-y-1">
                ...
                <flux:input
                    ...
                    id="icon-picker-{{ $pivotId }}_{{ $row }}_{{ $field->id }}.{{ $localeCode }}" data-icon-picker
